...
OSM data was previously downloaded. If the file is too large, an alternative is to download from [Interline](https://app.interline.io/osm_extracts/interactive_view).
...
Build the graph:
```{r build-graph}
r5_phoenix_cma <- setup_r5(data_path = r5_path, 
                            verbose = FALSE)
...
```{r set up network}
phoenix_network <- street_network_to_sf(r5_phoenix_cma)
```
...
```{r}
ttm_i <- travel_time_matrix(r5_phoenix_cma,
...
for(i in 1:n_origins){
  ...
  ttm_i <- bind_rows(ttm_i,travel_time_matrix(r5_phoenix_cma,
                                              origins = origins_i_tract[i,],
                                              destinations = destinations_j_h3,
                                              mode = "WALK",
                                              departure_datetime = as.POSIXct(strptime("2021-04-05 08:00:00", "%Y-%m-%d %H:%M:%S", tz = "EST5EDT")),
