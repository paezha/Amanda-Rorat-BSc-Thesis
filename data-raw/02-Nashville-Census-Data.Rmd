...
```{r}
nashville_tracts_df <- nashville_tracts |>
...
```{r}
nashville_tracts_df <- nashville_tracts_df |>
...
ggplot(nashville_tracts_df, aes(fill = area)) + 
  geom_sf() + 
  theme_void() +
  scale_fill_viridis_c(option = "turbo") +
  labs(fill = "Area of Walkshed
in Square Meters") +
  geom_sf(data = nashville_tracts, fill = NA, color = "purple")
```

```{r}
ggplot(nashville_tracts_df, aes(fill = black_prop)) + 
  geom_sf() +
  theme_void() +
  scale_fill_viridis_c(option = "turbo") +
...
```{r}
ggplot(nashville_tracts_df, aes(fill = white_prop)) + 
  geom_sf() + 
  theme_void() +
  scale_fill_viridis_c(option = "turbo") +
labs(fill = "Proportion of
...
```{r}
ggscatter(nashville_tracts_df, x = "black_prop", y = "area", add = "reg.line", shape = 19, size = 1, xlab = "Proportion of Black Population in Census Tract", ylab = "Area of Walkshed in Square Meters", font.label = c(5, "plain", "black")) +
  stat_cor(label.x = 0.75, label.y = 2000000) #label placement
```

```{r}
ggscatter(nashville_tracts_df, x = "white_prop", y = "area", add = "reg.line", shape = 19, size = 1, xlab = "Proportion of White Population in Census Tract", ylab = "Area of Walkshed in Square Meters", font.label = c(5, "plain", "black")) +
  stat_cor(label.x = 0.75, label.y = 2000000) #label placement
```

```{r}
ggscatter(nashville_tracts_df, x = "income", y = "area", add = "reg.line", shape = 19, size = 1, xlab = "Average Income in Census Tract", ylab = "Area of Walkshed in Square Meters", font.label = c(5, "plain", "black")) +
  stat_cor(label.x = 80000, label.y = 1500000) #label placement
```