...
```{r set up r5 path}
r5_path <- file.path(glue::glue(here::here(), "/data-raw/r5_graph/Tampa/"))
```
...
```{r}
origins_i_tract <- data.frame(id = tampa_tracts$GEOID,
                           origins_da_tampa |>
                          st_coordinates()) |>
  rename(lon = X, lat = Y) |>
...
The above chunk took 1.37 hours in Antonio's system Rogue and 3.07 hours in Amanda's system.

Retrieve the h3 areas as multipolygons:
```{r}
start.time <- Sys.time()

tampa_walksheds <- ttm_i |> 
  group_by(from_id) |>
...
The above chunk took about 1.18 h in Antonio's system Rogue and 1.94 hours in Amanda's system.

Obtain the GEOIDs of the walksheds:
```{r}
tampa_walkshed_IDs <- ttm_i |>
  group_by(from_id) |>
...
tampa_walksheds <- data.frame(GEOID = tampa_walkshed_IDs$from_id,
                         geometry = purrr::map_df(tampa_walksheds,
                            tibble::as_tibble)) |>
  st_as_sf(crs = 4326)
...
```{r}
test_unit = sample.int(nrow(tampa_walksheds), 1)

ggplot() +
  geom_sf(data = tampa_tracts |>
            filter(GEOID == tampa_walksheds$GEOID[test_unit]),
          color = "black",
          fill= "lightgray") + 
  geom_sf(data = tampa_walksheds[test_unit,],
          color = "blue",
          fill = NA,
          linewidth = 1) #+
  #geom_sf(data = roads)
...
# Origins
save(tampa_walksheds, 
     file = glue::glue(here::here(),
                       "/data/tampa_walksheds.rda"),
     compress = "bzip2")
