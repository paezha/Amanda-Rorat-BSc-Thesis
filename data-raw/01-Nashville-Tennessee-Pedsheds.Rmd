---
title: "Nashville Tennessee Ped Sheds"
---
...
```{r set up r5 path}
r5_path <- file.path(glue::glue(here::here(), "/data-raw/r5_graph/Nashville/"))
...
```{r prepare-inputs}
origins_da_nashville <- nashville_tracts |>
  st_make_valid() |>
  st_centroid() |>
...
```{r}
origins_i_tract <- data.frame(id = nashville_tracts$GEOID,
                           origins_da_nashville |>
                          st_coordinates()) |>
  rename(lon = X, lat = Y) |>
...
The above chunk took 32.44 mins in Amanda's system.

Retrieve the h3 areas as multipolygons:
```{r}
start.time <- Sys.time()

nashville_walksheds <- ttm_i |> 
  group_by(from_id) |>
...
The above chunk took about 22.77 minsin Amanda's system.

Obtain the GEOIDs of the walksheds:
```{r}
nashville_walkshed_IDs <- ttm_i |>
  group_by(from_id) |>
...
nashville_walksheds <- data.frame(GEOID = nashville_walkshed_IDs$from_id,
                         geometry = purrr::map_df(nashville_walksheds,
                            tibble::as_tibble)) |>
  st_as_sf(crs = 4326)
...
```{r}
test_unit = sample.int(nrow(nashville_walksheds), 1)

ggplot() +
  geom_sf(data = nashville_tracts |>
            filter(GEOID == nashville_walksheds$GEOID[test_unit]),
          color = "black",
          fill= "lightgray") + 
  geom_sf(data = nashville_walksheds[test_unit,],
          color = "blue",
          fill = NA,
          linewidth = 1) #+
  #geom_sf(data = roads)
