...
```{r}
jacksonville_cbsa <- core_based_statistical_areas(year = 2020) |>
  filter(str_detect(NAME, "Jacksonville, FL"))
```
...
```{r}
jacksonville_city <- urban_areas(year = 2020) |> 
  filter(str_detect(NAME10, "Jacksonville, FL"))
```
...
ggplot() +
  geom_sf(data = fl_tracts,
          fill = "lightgrey") +
  geom_sf(data = jacksonville_cbsa,
          color = "red",
          fill = NA) +
...
```{r}
jacksonville_urban <- st_filter(fl_city, jacksonville_cbsa)
```
...
ggplot() +
  geom_sf(data = jacksonville_cbsa,
          fill = "lightgray") +
  geom_sf(data = jacksonville_urban,
          color = "blue",
          fill = NA) +
  theme_minimal()
```
...
Filter Houston:
```{r}
jacksonville_urban <- jacksonville_urban |>
  filter(NAME10 == "Jacksonville, FL")
```
...
```{r}
jacksonville_tracts <- fl_tracts |>
  st_filter(jacksonville_urban, 
