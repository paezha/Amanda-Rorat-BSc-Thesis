...
```{r set up r5 path}
r5_path <- file.path(glue::glue(here::here(), "/data-raw/r5_graph/Jacksonville/"))
...
```{r prepare-inputs}
origins_da_jacksonville <- jacksonville_tracts |>
  st_make_valid() |>
  st_centroid() |>
...
```{r}
origins_i_tract <- data.frame(id = jacksonville_tracts$GEOID,
                           origins_da_jacksonville |>
                          st_coordinates()) |>
  rename(lon = X, lat = Y) |>
...
The above chunk took 30.133 mins in Amanda's system.

Retrieve the h3 areas as multipolygons:
```{r}
start.time <- Sys.time()

jacksonville_walksheds <- ttm_i |> 
  group_by(from_id) |>
...
The above chunk took about _ in Amanda's system.

Obtain the GEOIDs of the walksheds:
```{r}
jacksonville_walkshed_IDs <- ttm_i |>
  group_by(from_id) |>
...
jacksonville_walksheds <- data.frame(GEOID = jacksonville_walkshed_IDs$from_id,
                         geometry = purrr::map_df(jacksonville_walksheds,
                            tibble::as_tibble)) |>
  st_as_sf(crs = 4326)
...
```{r}
test_unit = sample.int(nrow(jacksonville_walksheds), 1)

ggplot() +
  geom_sf(data = jacksonville_tracts |>
            filter(GEOID == jacksonville_walksheds$GEOID[test_unit]),
          color = "black",
          fill= "lightgray") + 
  geom_sf(data = jacksonville_walksheds[test_unit,],
          color = "blue",
          fill = NA,
          linewidth = 1) #+
  #geom_sf(data = roads)
