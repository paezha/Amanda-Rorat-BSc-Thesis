---
title: "Atlanta Georgia Ped Sheds"
---
...
```{r set up r5 path}
r5_path <- file.path(glue::glue(here::here(), "/data-raw/r5_graph/Atlanta/"))
...
```{r prepare-inputs}
origins_da_atlanta <- atlanta_tracts |>
  st_make_valid() |>
  st_centroid() |>
...
```{r}
origins_i_tract <- data.frame(id = atlanta_tracts$GEOID,
                           origins_da_atlanta |>
                          st_coordinates()) |>
  rename(lon = X, lat = Y) |>
...
The above chunk took 1.28 hours in Antonio's system Rogue and 2.99 hours in Amanda's system.

Retrieve the h3 areas as multipolygons:
```{r}
start.time <- Sys.time()

atlanta_walksheds <- ttm_i |> 
  group_by(from_id) |>
...
The above chunk took about 52 min in Antonio's system Rogue and 1.85 hours in Amanda's system.

Obtain the GEOIDs of the walksheds:
```{r}
atlanta_walkshed_IDs <- ttm_i |>
  group_by(from_id) |>
...
atlanta_walksheds <- data.frame(GEOID = atlanta_walkshed_IDs$from_id,
                         geometry = purrr::map_df(atlanta_walksheds,
                            tibble::as_tibble)) |>
  st_as_sf(crs = 4326)
...
```{r}
test_unit = sample.int(nrow(atlanta_walksheds), 1)

ggplot() +
  geom_sf(data = atlanta_tracts |>
            filter(GEOID == atlanta_walksheds$GEOID[test_unit]),
          color = "black",
          fill= "lightgray") + 
  geom_sf(data = atlanta_walksheds[test_unit,],
          color = "blue",
          fill = NA,
          linewidth = 1) #+
  #geom_sf(data = roads)
